...
func alarm() {
	...
	ossurl := fmt.Sprintf("http://%s.oss-cn-hangzhou-internal.aliyuncs.com/jvm/%s/%s/%s-%s", bucketName, env, folder, podId, postfix) //建议OSS内网，"oss-cn-shanghai-internal"更换成自己的endpoint[需要修改]
